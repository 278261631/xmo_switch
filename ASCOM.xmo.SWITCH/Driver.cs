...
{
    //
    // Your driver's DeviceID is ASCOM.xmo.Switch
    //
    // The Guid attribute sets the CLSID for ASCOM.xmo.Switch
    // The ClassInterface/None attribute prevents an empty interface called
    // _xmo from being created and used as the [default] interface
    //
    ...
    /// ASCOM Switch Driver for xmo.
    /// </summary>
    [Guid("c277eb0c-69ac-4d55-b907-d80032f3947a")]
    ...
    public class Switch : ISwitchV2
    {
        ...
        /// </summary>
        internal static string driverID = "ASCOM.xmo.Switch";
        // TODO Change the descriptive string for your driver then remove this line
        /// <summary>
        /// Driver description that displays in the ASCOM Chooser.
        /// </summary>
        private static string driverDescription = "ASCOM.xmo.Switch";
        ...
        internal static TcpClientWrapper tcpClient;
        internal static string client_ip;
        internal static int port;
        internal static string client_ipProfileName = "IP:"; // Constants used for Profile persistence
        internal static string client_ipDefault = "192.168.1.166";
        internal static string port_ProfileName = "port:";
        internal static string port_Default = "1234";
        internal static List<bool> allStatus = new List<bool>();

        private static System.Timers.Timer commonTimer;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="xmo"/> class.
        /// Must be public for COM registration.
        /// </summary>
        public Switch()
        {
            tl = new TraceLogger("", "xmo");
            ...
            commonTimer = new System.Timers.Timer(500);
            commonTimer.Elapsed += OnTimedEvent;
        ...
        private void OnTimedEvent(object sender, ElapsedEventArgs e)
        {
            if (IsConnected)
            {
                ...
                commandParams.Add("cmd_id", 4);
                commandParams.Add("cmd_name", "temp");
                string JSONresult;
                JSONresult = JsonConvert.SerializeObject(commandParams);
                Console.WriteLine(JSONresult);
                tcpClient.Send("state=?");
                string result = tcpClient.Receive();
                Console.WriteLine("<-: " + result);
                decodeResult(result);
            }
        }

        private static bool decodeResult(string result)
        {
            try
            {
                ...
                Trace.WriteLine(String.Format("  {0}", result));
                if (values.ContainsKey("output"))
                {
                    string output_str = values["output"].ToString();
                    for (int i = 0; i < allStatus.Count; i++)
                    {
                        allStatus[i] = output_str.Substring(i, 1).Equals("1") ;
                    }


                    Trace.WriteLine(String.Format(" output {0}", values["output"]));
                }
                if (values.ContainsKey("input"))
                {
                    Trace.WriteLine(String.Format(" input {0}", values["input"]));
                }
            ...
            {
                Trace.WriteLine(String.Format(" input {0}", ex.Message));
                return false;
            }
        ...
        public bool Connected
        {
            ...
            set
            {
                ...
                if (value)
                {
                    try
                    {
                        tcpClient = new TcpClientWrapper(client_ip, port);
                        
                        LogMessage("Connected Set", "Connecting to port {0}  {1}", client_ip, port);
                        tcpClient.Send("state=?");
                        string result = tcpClient.Receive();
                        Console.WriteLine("<-: " + result);
                        ...
                        {
                            this.numSwitch = (short)output_str.Length;
                            allStatus.Clear();
                            ...
                            connectedState = true;
                            Trace.WriteLine(String.Format("  {0}", values["sn"]));
                            commonTimer.Start();
                        }
                    }
                    catch (Exception ex)
                    {
                        ...
                        commonTimer.Stop();
                        LogMessage("Connected Set", "Connecting to port {0}  {1} {2}", client_ip, port, ex.Message);
        ...
        public string Name
        {
            get
            {
                string name = "xmoSwitch"+client_ip.ToString();
        ...
        public void SetSwitch(short id, bool state)
        {
            ...
            string control_cmd = "setr=";
            for (int i = 0; i < allStatus.Count; i++)
            {
                if (id == i)
                {
                    control_cmd += state ? "1" : "0";
                }
                else
                {
                    control_cmd += "x";
        ...
        internal void ReadProfile()
        {
            using (Profile driverProfile = new Profile())
            {
                ...
                client_ip = driverProfile.GetValue(driverID, client_ipProfileName, string.Empty, client_ipDefault);
                
                
                port = Convert.ToInt32(driverProfile.GetValue(driverID, port_ProfileName, string.Empty, port_Default));
            }
        }
